...
folder_name = "svm_ccd-train_ccd-test"
...
for class_label in range(np.max(ccd_train_label)+1):
    ...
    print(" OCCD train data = ", (ccd_train_data.shape[0]))
    print("CCD validation data  = ", (ccd_test_data.shape[0]))
...
svm_rbf_classifier.fit(ccd_train_data, ccd_train_label[:, 0])
predicted_svm_rbf_val_label = svm_rbf_classifier.predict(ccd_test_data)
acc_svm_rbf = accuracy_score(ccd_test_label, predicted_svm_rbf_val_label)*100
f1score_svm_rbf = f1_score(ccd_test_label, predicted_svm_rbf_val_label, average="macro")
report_svm_rbf = classification_report(ccd_test_label, predicted_svm_rbf_val_label, target_names=target_names)
...
confusion_matrix_svm_rbf = cm(ccd_test_label, predicted_svm_rbf_val_label)
